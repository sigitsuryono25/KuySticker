package com.surelabsid.whatsappapi.utils;
...
import com.surelabsid.whatsappapi.whatsapp_api.ContentFileParser;
...
public class StickerPacksManager {
    ...
    public static List<Sticker> saveStickerPackFilesLocally(String identifier, List<HashMap<String, Uri>> stickersUries, Context context) {
        ...
        for (HashMap<String, Uri> maps : stickersUries) {
            for (Map.Entry<String, Uri> entry : maps.entrySet()) {
                String key = entry.getKey();
                Sticker sticker = new Sticker(key + ".webp", null);
                stickerList.add(sticker);
                saveStickerFilesLocally(sticker, maps.get(key), stickerPath, context);
            }
        }
        return stickerList;
    }
